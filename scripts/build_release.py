...
def main() -> int:
    ...
    versioned_hacs_zip = output / f"maika-v{version}-hacs.zip"
    versioned_manual_zip = output / f"maika-v{version}-manual.zip"
    _write_zip(hacs_zip, manual=False)
    _write_zip(manual_zip, manual=True)
    shutil.copy2(hacs_zip, versioned_hacs_zip)
    shutil.copy2(manual_zip, versioned_manual_zip)

    checksums = output / "SHA256SUMS.txt"
    checksums.write_text(
        "".join(
            f"{_sha256(path)}  {path.name}\n"
            for path in (hacs_zip, manual_zip, versioned_hacs_zip, versioned_manual_zip)
        ),
        encoding="utf-8",
    )
